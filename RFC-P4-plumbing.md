...
| P4.8 | `tenon doctor` (one-shot, human-triggered install/env diagnostics: toolchain, sandbox backends, ports, state integrity) — does NOT replace the guardian's live health loop; both reuse ONE shared probe list (each probe = name + pure check -> ok/fail+detail): the guardian runs it periodically to drive auto-reset, doctor runs it once offline plus install-only checks. Small refactor: converge the hardcoded guardian probes (P3.5b) into that shared list. docs + REVIEW-P4 (perf tables incl. UI latency), NOTES update | all gates; secret scan; no leftover containers |
