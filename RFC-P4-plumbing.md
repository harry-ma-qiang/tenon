...
|---|---|---|
| P4.0 | `rs/bus` (envelope, Hub, tracing Layer, `t:"ev"` frames), kv facade (memory + durable + lease/watch/revision), blob facade over existing blobs, RPCs `bus.publish/subscribe`, `kv.*`, `blob.*` | budgets in section 4 measured by a bench test; durable replay after kill -9; existing suites green |
